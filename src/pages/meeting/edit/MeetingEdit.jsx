...
import { Alert, Image, View, Text, ScrollView, StyleSheet, TextInput, TouchableOpacity, Pressable} from 'react-native';
...
export default function MeetingEdit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMeetingName(json.data.info.name);
        setMeetingTopic(json.data.info.title);
        setMeetingType(json.data.info.type);
        setMeetingDescription(json.data.info.content);
        const apiMeetingCategory = (json.data.info.categories).map((type) => MEETING_CATEGORY[type]);
        setSelectedMeetingTypes(apiMeetingCategory);
        setTags(json.data.info.features);
        setSelectedDays(json.data.info.days);
        
        setParticipantMethod(json.data.info.applicationMethod);
        console.log(participantMethod);
        setadditionalInfo(json.data.info.location);
        setSelectedDate(json.data.info.startDate);
        setStartDate(json.data.info.startDate);
        setEndDate(new Date(json.data.info.endDate));
        // setStartTime(json.data.info.startTime);
        // setEndTime(json.data.info.endTime);
        // setMeetingDescription(json.data.info.meta);
      } catch (error) {
  ...
  const APPLICATION_METHOD2 = {
    'FIRST_COME_FIRST_SERVED':'선착순 모임',
    'LEADER_ACCEPT':'모임장 수락/거절'
  }
...
const removeTag = (tagToRemove) => {
  ...
  const hideStartPicker = () => {
    ...
    const getMarkedDates = () => {
      ...
      if (Types[meetingType] === '일회성 모임' && selectedDate) {
        markedDates[selectedDate] = { selected: true, marked: true, dotColor: 'blue' };
      } else if (Types[meetingType] === '정기 모임' && startDate && endDate) {
  ...
  return (
    // <Layout screen={PAGES.MEETING_CREATE}>
      ...
      <RadioButtonGroup
        ...
        selectedOption={Types[meetingType]}
        onSelect={()=>{}}
...
<>
     ...
     <View style={styles.timePickerContainer}>
      ...
      <RadioButtonGroup
        ...
        selectedOption={APPLICATION_METHOD2[participantMethod]}
        onSelect={()=>{}}
